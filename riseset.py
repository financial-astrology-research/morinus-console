...
#		self.printRiseSet(pls)
	...
	def calcTimes(self):
		...
		for i in range(planets.Planets.PLANETS_NUM):#Nodes are excluded
			...
			#From GMT to Local
#			JDRise += self.offs
			year, month, day, hr = swisseph.revjul(JDRise, self.calflag)
			ar.append(hr)

#			JDMC += self.offs
			year, month, day, hr = swisseph.revjul(JDMC, self.calflag)
			ar.append(hr)

#			JDSet += self.offs
			year, month, day, hr = swisseph.revjul(JDSet, self.calflag)
			ar.append(hr)

#			JDIC += self.offs
			year, month, day, hr = swisseph.revjul(JDIC, self.calflag)
			ar.append(hr)
