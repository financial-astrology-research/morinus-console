...
class PlacidianSAPD(placidiancommonpd.PlacidianCommonPD):
  ...
  def calcInterPlanetary(self, mundane):
    ...
    # iterate over all the chart planets
    for p in range(len(self.chart.planets.planets)):
      # looks like promplanets contains the promisor planets that are checked
      # in the primary directions options if not selected the omit it
      ...
      # take the data of the current iterate planet, there is the ephemerides
      # data and other important stuff
      plprom = self.chart.planets.planets[p]
      # each class has a list of array positions for each variable for example
      # the PLACSPECULUM is stored at speculums array position PLACSPECULUM = 0
      # in the same manner Planet class store the right ascencion at position RA = 2
      ...
      # directions not mundane and subzodical options are not latitude promissor or latitude both
